...
include_once("HistoriaClinica.php");

// Ejemplo de uso de la Fachada:
//
// $fachada = new HistoriaClinica();
// $fachada->registrarHistoriaClinica($_POST, $_FILES);
//
// Actualmente el sistema continúa utilizando la lógica
// implementada directamente en subirHC.php.


include "conectar.php";//conectar a la base de dato
include_once("ValidadorHC.php");

// Validar datos antes de procesar
$errores = ValidadorHC::validar($_POST, $_FILES);
if (count($errores) > 0) {
...
$foto= $_FILES['archivo']['name'];
	echo "hola". $foto;
	$Sql="INSERT INTO historia_clinica (nombre_apellido, dni, domicilio, edad, ocupacion, obra_social, motivo_de_consulta, vive_con, contencion,  que_contencion_tiene, fecha, file_name) VALUES
	('$Nombre', '$DNI', '$Domicilio', '$Edad', '$Ocupacion', '$OS', '$motivo', '$vive_con', '$conteccion',  '$con', '$fecha', '$foto')";
	
	if(!$resultado = mysql_query($Sql)){
		echo "<script>
                alert('Error al tratar de guardar Paciente');
                window.location= 'CargarHC.php'
    		</script>";
		exit();
	}else{
		$carpeta="Fotos/";
...
$verificar_usuario=mysql_query("SELECT id_paciente 
								FROM historia_clinica 
								WHERE nombre_apellido LIKE '$Nombre' AND dni LIKE '$DNI' AND domicilio LIKE '$Domicilio' AND edad LIKE '$Edad' AND ocupacion LIKE '$Ocupacion' AND obra_social LIKE '$OS' AND motivo_de_consulta LIKE '$motivo' AND	vive_con LIKE '$vive_con' AND contencion LIKE '$conteccion' AND	que_contencion_tiene LIKE '$con' AND fecha LIKE '$fecha' AND	file_name LIKE '$Nombrefoto'");
...
if ($var==1){
	$Sql="INSERT INTO  familiares (id_paciente, madre, padre, hermanos, otrosF) VALUES
	                              ('$substr','$Madre', '$padre', '$hnos', '$otrosF')";
